...
package org.fao.geonet.schema.<%= name.replace('.', '_') %>;
...
import static org.fao.geonet.schema.<%= name.replace('.', '_') %>.<%= name.toUpperCase().replace('.', '') %>Namespaces.*;

/**
 * SchemaPlugin for <%= name %> metadata standard.
 *
 */
public class <%= name.toUpperCase().replace('.', '') %>SchemaPlugin
    ...
    public <%= name.toUpperCase().replace('.', '') %>SchemaPlugin() {
